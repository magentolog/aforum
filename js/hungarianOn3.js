 ...
 */
function hungarianOn3(costMatrix, isProfit) {
  var cloneMatrix = clone2d(costMatrix);
  var ap = new AP(cloneMatrix, isProfit);
...
AP.prototype.makeItProfit = function () {
  ...
  for (i = 0; i < this.costMatrix.length; i++) {
    ...
    for (j = 0; j < row.length; j++) {
      row[j] = (row[j] === 0) ? this.BIG_M : (biggestVal - row[j])
    }
  }
};
